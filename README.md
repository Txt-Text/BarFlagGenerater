...
右边实时显示生成的旗帜效果，下方标注 `900 × 600 · 3 条色条 · 比例和 3`。任何输入变化都会立刻重画，不用点生成。

#### 4. 预设

- **下拉框** —— 列出扫描到的所有预设，选中后点「载入」
- **载入** —— **把预设的内容填进上面的表单，但不会生成图片**。分辨率、色条、输出文件名都会被替换，请以载入后的界面为准
...
| `-s, --strips LIST` | 色条列表，如 `"#FF0000 1,#FFFFFF 1.5"` |
...
- `strips:` 这一行开始是色条段（写 `strips` 开头即可，冒号可有可无）
- 段外以 `#` 开头的行一律当注释
- 段内以 `#` 开头的行会**先尝试当色条解析**：`#` 后面到第一个空白/冒号为止如果是合法的 3/4/6/8 位十六进制，就当色条；否则当注释
